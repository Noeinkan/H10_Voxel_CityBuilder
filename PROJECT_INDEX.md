...
| [SelectionOutline.test.ts](src/engine/SelectionOutline.test.ts) | Coperchio e montanti solo su cio' che ha un'altezza, fascia mai dentro la collina, nessuna riscrittura a riquadro invariato, tempo che rientra | — |
...
| [IsoCameraController.test.ts](src/engine/IsoCameraController.test.ts) | Perno della rotazione sotto il cursore, orbita col tasto centrale che non sposta l'inquadratura, riaggancio della griglia dallo scatto piu' vicino, orbita attorno al target con inclinazione clampata, pan che sale lungo il soggetto, punta di una torre di settecento voxel dentro lo schermo e ripristino identico | — |
...
| File | Ruolo | Esporta |
| --- | --- | --- |
| [forecast.test.ts](src/sim/forecast.test.ts) | Arrotondamento, il silenzio quando la scorta è ferma, la spesa dei cantieri fuori dal delta e la decadenza geometrica delle case libere | — |
...
| [engine/mesher/microCrown.test.ts](src/engine/mesher/microCrown.test.ts) | Il coronamento misurato da solo: tre torri, tre profili di filo e nessuno che porti quello di un'altra; `facadeUnder` attraversa cio' che non e' un uso e si ferma nel vuoto; costo per perimetro e non per cella; ogni prop poggia sul calpestio vero, vassoio compreso |
...
| [engine/mesher/microThreshold.test.ts](src/engine/mesher/microThreshold.test.ts) | L'attacco a terra: compare solo davanti a un ingresso esposto, segue l'arretramento della soglia, costa per ingresso e non per cella, e senza marciapiede sotto non mette il gradino |
...
| [src/engine/street/StreetCameraController.test.ts](src/engine/street/StreetCameraController.test.ts) | Occhio fermo sotto qualunque gesto, clamp di inclinazione e campo visivo, drag su ogni tasto del mouse, sensibilita' che segue il teleobiettivo |
| [src/engine/street/streetEye.test.ts](src/engine/street/streetEye.test.ts) | Rifiuto dell'acqua e del vuoto, salita sui tetti, inclinazione che attraversa lo zero, scatola d'ombra che puo' solo rimpicciolire |
...
| [ui/MainMenuModel.test.ts](src/ui/MainMenuModel.test.ts) | Le quattro sezioni esaustive sull'unione e senza Resume, il nome dell'autosave contro quello degli slot a mano, il riassunto asserito sulle parti che non sono la data, e la riga di identita' senza un numero di versione a mano |
| [src/ui/SwatchOverlayModel.test.ts](src/ui/SwatchOverlayModel.test.ts) | La scheda del campionario: stato vuoto, righe del soggetto chiuse da ingombro e altezza, dettaglio solo sulla matrice, come si molla la scelta, referto del voxel in tre righe |
| [src/ui/TitleScreenModel.test.ts](src/ui/TitleScreenModel.test.ts) | Continue solo con un autosalvataggio e una sola voce grande, la parola della prima voce, Load spento su elenco vuoto, il seed vuoto che vale «sorteggiane uno» e lo zero che non e' un seed |
...
| [world/arcology/shaft.test.ts](src/world/arcology/shaft.test.ts) | Il pozzo contro il cavedio, la bocca che le passerelle attraversano senza chiudere, il vuoto che tocca il bordo e non conta |
...
| [world/buildings/cityDigest.test.ts](src/world/buildings/cityDigest.test.ts) | L'impronta digitale di una citta' matura a parita' di seme: quali edifici nascono, dove, di che livello e in che ordine. La rete con cui un'ottimizzazione di `findLot` dimostra di non aver spostato niente |
...
| [world/buildings/lotMemo.test.ts](src/world/buildings/lotMemo.test.ts) | Il memo dei lotti senza mondo: cosa ricorda, cosa dimentica al reset, e che nessuna coppia isolato-lato scavalchi un'altra nella chiave impacchettata |
| [world/buildings/sunkenSites.test.ts](src/world/buildings/sunkenSites.test.ts) | Che l'isola offra davvero la roccia che le ricette chiedono, su tre seed: e' l'allarme contro la profondita' tarata a occhio |
