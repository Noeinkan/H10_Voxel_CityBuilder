...
| [InfluenceOverlay.ts](src/engine/InfluenceOverlay.ts) | Portata dei catalizzatori: velatura a gradiente decimata, isolinee ai quarti e contorno tracciati con marching squares sui dati veri del campo; perimetri dei settori e cache del campo del cursore | `InfluenceOverlay`, `ReachSummary` |
...
| [growthScene.ts](src/game/growthScene.ts) | Cablaggio esclusivo di `grow=1`: tick, Builder e animazione; espone la gomma (`demolish`, `demolishAt`, `demolishSurvey`, `demolishPreview`, `undoDemolition`) al gioco | `GrowthScene`, `GrowthStats` |
