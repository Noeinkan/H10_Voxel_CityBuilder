...
| [src/sim/materials.ts](src/sim/materials.ts) | Rendiconto dei materiali — prodotti, comprati, spesi —, capacità economica dei livelli e costi dei cantieri verticali |
...
| [seasons.ts](src/sim/seasons.ts) | L'anno: fase, stagione e moltiplicatore del raccolto, tutti funzione pura di `tickCount`. Media annua esattamente uno, cosi' chi pianta continua a dimensionare la campagna su un numero onesto | `SEASON`, `SEASON_NAMES`, `SEASON_COUNT`, `yearPhaseAt`, `seasonAt`, `harvestFactorAt`, `Season` |
...
| [towerProspect.ts](src/sim/towerProspect.ts) | Quale edificio industriale è più vicino a diventare torre idroponica, e quale soglia gli manca | `nearestTowerProspect`, `TowerProspect` |
| [trade.ts](src/sim/trade.ts) | Import/export aggregato sbloccato dal porto, nei due versi: cibo in entrata, materiali in entrambi | `resolveExternalTrade`, `foodImportShareOf`, `tradeLinksOf`, `TRADE_MODES`, `TradeMode`, `TradeReport` |
