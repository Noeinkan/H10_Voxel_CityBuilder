...
| File | Ruolo | Esporta |
| --- | --- | --- |
| [archDriver.ts](src/world/buildings/archDriver.ts) | La passata delle campate: la sola che scrive su due record insieme, perche' un arco e' un patto e non un gesto | `ArchDriver` |
| [archPlan.ts](src/world/buildings/archPlan.ts) | La regola della campata: da due dirimpettai maturi ai due bracci che si incontrano a meta' del vuoto, o al motivo per cui non si incontrano. Pura | `planArch`, `ARCH_REFUSALS`, `BuildingArch`, `ArchPair`, `ArchSide`, `ArchQuery`, `ArchResult`, `ArchRefusal` |
| [archStamp.ts](src/world/buildings/archStamp.ts) | Il braccio scritto nei voxel: il corso con il rinfianco, la materia presa dal muro d'imposta, e le due letture — il solo braccio per la comparsa, corpo piu' braccio per la cancellazione | `archArm`, `withArch`, `ArchArm`, `ArchHost` |
...
| [config/arch.ts](src/world/buildings/config/arch.ts) | Quando due fronti si toccano e con che sagoma: soglia di torre, vuoto ammesso, spessore del corso, rinfianco, franco e tolleranza di fascia | `ARCH` |
...
| [config/fusion.ts](src/world/buildings/config/fusion.ts) | Quando due edifici smettono di essere due: soglia derivata dal primo gradino d'impronta, quanti se ne assorbono, e il lato oltre cui la cancellazione non sta piu' nel budget | `FUSION` |
...
| [fusion.ts](src/world/buildings/fusion.ts) | La regola della fusione: chi puo' assorbire chi, e gli usi che il sopravvissuto eredita. Pura, e legge la classificazione di `clearance.ts` invece di scriversene una seconda | `planFusion`, `FUSION_REFUSALS`, `FusionMember`, `FusionQuery`, `FusionPlan`, `FusionResult`, `FusionRefusal` |
| [fusionDriver.ts](src/world/buildings/fusionDriver.ts) | Due edifici che si incontrano e diventano uno: apre i cantieri di sgombero sui vicini, aspetta che finiscano e riscrive il sopravvissuto sull'impronta larga | `FusionDriver` |
...
| [multiStamp.ts](src/world/buildings/multiStamp.ts) | La sagoma di un edificio che sta su piu' sedimi: i corpi composti su una tela sola, i sedimi in piu' disegnati con un sotto-seme del sopravvissuto, e l'arco dipinto sopra alla fine | `partedStamp` |
...
| [src/world/buildings/roadDriver.ts](src/world/buildings/roadDriver.ts) | La passata del tracciato: tiene la rete organica, la manda sulla coda di superficie quando i poli cambiano, e tira il capillare verso un edificio appena nato | `RoadDriver` |
