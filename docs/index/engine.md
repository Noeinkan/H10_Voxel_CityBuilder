...
| [season.ts](src/engine/season.ts) | L'anno visto dal renderer: rigoglio, oro e brina come tre curve sfasate, e il tema piegato alla stagione — prato e aria, nessuna geometria. Senza Three, come `daylight.ts` | `SEASON_LOOK`, `seasonMood`, `seasonColors`, `withSeason`, `SeasonMood` |
...
| [microGarden.ts](src/engine/mesher/microGarden.ts) | Il verde di copertura come luogo: fioriere di bordo, chiome, cespugli. Riconosce il giardino pensile senza un linguaggio di superficie nuovo — erba, aria sopra, edificio sotto — e la lista di celle la riempie `collectSurfaceCells` nel ramo che gia' scartava il `plain` | `isRoofGarden`, `appendGardenDetail` |
