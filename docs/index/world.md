...
| File | Ruolo | Esporta |
| --- | --- | --- |
| [congestion.ts](src/world/congestion.ts) | L'ingorgo come geografia: densità costruita per tessere, sfocata sul vicinato, alleggerita dai ruoli che muovono gente. Dice anche **se** il carico è cambiato, che è ciò che decide se rifare il campo | `CongestionMap`, `transitSourcesOf`, `CongestionLookup`, `TransitSource` |
...
| [src/world/roads/terrainCost.ts](src/world/roads/terrainCost.ts) | La parte continua del costo di una colonna: pendenza e campo di divagazione. E' cio' che rompe i pareggi di un terreno a gradini, dove ogni cammino costa identico e la ricerca restituisce la diagonale canonica | `terrainPenalty`, `wanderAt` |
